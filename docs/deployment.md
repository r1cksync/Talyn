...
Backend startup runs migrations and checkpoint initialization under a PostgreSQL advisory lock. Production rejects demo adapters. Development SES permits only the sender and simulator by default. For other test recipients, verify each address with SES in Mumbai, set repository variable `TALYN_ADDITIONAL_EMAIL_RECIPIENTS` to a comma-separated list (up to 20 addresses), and redeploy. Manual CDK deployments use `-c additionalEmailRecipients=candidate@example.com`. Include any simulator aliases used for manager notifications. CDK also grants SendEmail access to those specific recipient identity ARNs, which SES checks in sandbox mode, while preserving the fixed sender condition. The allowlist stays enabled and SES sandbox restrictions still apply. Enable `TALYN_LIVE_EMAIL` deliberately only after production approval and recipient review.
