...
2. **CloudFront account verification.** Distribution creation returned HTTP 403: the account must be verified through AWS Support before adding CloudFront resources. Request ID `956730e5-3dce-42aa-87ed-30f5b88af101`. CloudFormation rolled back that runtime attempt. The deployed user-approved IP fallback uses a small AWS nginx gateway and automatically renewed, trusted IP certificate; no domain purchase is required.
...
4. **Alert recipient selected.** The same user-controlled inbox will receive budget/operational alerts; its SNS subscription is confirmed. Alerts are not a hard spending cap.
