...
2. **No-domain HTTPS resolved in infrastructure.** The user has no domain; the stack now supports an AWS-generated CloudFront HTTPS address with a private ALB VPC origin. No domain purchase, public IP HTTP workaround or DNS ownership is required. This path is synthesized/tested but not deployed.
