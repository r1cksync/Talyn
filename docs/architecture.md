...
API/worker startup serializes migrations with a session advisory lock, acquired through bounded `pg_try_advisory_lock` polling on an autocommit connection. A blocking lock query can retain a snapshot that prevents LangGraph's concurrent checkpoint indexes from finishing, while waiting for the index creator's advisory lock. Two fresh processes are tested against a unique PostgreSQL schema to cover this startup race. See [PostgreSQL concurrent index creation](https://www.postgresql.org/docs/16/sql-createindex.html).
...
* One origin routes `/api/*` to FastAPI and everything else to Next.js through the HTTPS gateway and private ALB (or an optional custom-domain HTTPS ALB). HttpOnly manager and candidate cookies are distinct; mutation requests check Origin and a CSRF header. Invitation secrets live in URL fragments, then exchange for an email verification challenge.
...
* A deterministic local adapter exercises the workflow without AWS. Production rejects demo mode. Demo speech input is explicitly synthetic. AWS mode uses the selected Groq provider (or optional Bedrock Converse), Polly, and real Transcribe Streaming.
...
Manager → Cognito → session → tenant API → PostgreSQL/outbox → SQS → worker → LangGraph/Groq (optional Bedrock) → approved plan.
...
Initial preflight found root credentials, SES sandbox and no verified identities. The sender is now verified, the alert subscription confirmed, and live Groq/AWS workflow acceptance passed. Bedrock account authorization remains blocked. See [acceptance evidence](acceptance.md) and [account blockers](aws-blockers.md) for current status.
