...
from .models import (Application, AsyncJob, AuthSession, Candidate, DeliveryEvent, Document, EmailDelivery,
    Extraction, InterviewSession, Invitation, RateBucket, RecordingObject, Usage, VideoObservation, now)
...
def extract_document(org_id, document_id):
    with SessionLocal() as db:
        ...
        if document.status == "ready":
            return
        key, size, sha, kind, app_id = document.object_key, document.size, document.sha256, document.content_type, document.application_id
    try:
        ...
        if len(result["text"].strip()) < 30:
            ...
            job_id = textract.start_document_text_detection(DocumentLocation={"S3Object": {"Bucket": settings().bucket, "Name": key}},
                                                            ClientRequestToken=sha)["JobId"]
            response = None
            for _ in range(12):
            ...
            sources = [{"ref": f"page:{b.get('Page', 1)}:line:{i}", "text": b["Text"]} for i, b in enumerate(blocks) if b["BlockType"] == "LINE"]
            result = {"text": "\n".join(s["text"] for s in sources), "sources": sources, "warnings": ["Extracted with OCR; review for recognition errors."]}
        ...
        with SessionLocal.begin() as db:
            ...
            if not db.scalar(select(Extraction.id).where(Extraction.document_id == doc.id, Extraction.org_id == org_id)):
...
def verify_recording(org_id, recording_id):
    with SessionLocal() as db:
        ...
        result = subprocess.run([binary, "-v", "error", "-xerror", "-i", "pipe:0", "-t", "36", "-map", "0:v:0", "-f", "null", "-"],
...
def observe_frame(org_id, session_id, dedupe_key):
    ...
    if settings().mode == "demo":
        ...
        faces = aws("rekognition").detect_faces(Image={"S3Object": {"Bucket": settings().bucket, "Name": key}}, Attributes=["DEFAULT"])["FaceDetails"]
    kinds = []
    if not faces:
    ...
    with SessionLocal.begin() as db:
        for kind, confidence in kinds:
            prior = db.scalar(select(VideoObservation).where(VideoObservation.session_id == session_id,
                VideoObservation.org_id == org_id, VideoObservation.kind == kind).order_by(VideoObservation.end_ms.desc()))
            ...
            if prior and offset-prior.end_ms <= settings().frame_interval_seconds*1500:
                ...
                prior.visible = prior.sample_count >= 2 and prior.end_ms-prior.start_ms >= settings().frame_interval_seconds*1000
                prior.confidence = min(prior.confidence, confidence)
            else:
                db.add(VideoObservation(org_id=org_id, session_id=session_id, kind=kind, start_ms=offset,
                    end_ms=offset, confidence=confidence, evidence_key=key))


def send_delivery(org_id, delivery_id):
    with SessionLocal.begin() as db:
        delivery = owned(db, EmailDelivery, delivery_id, org_id, lock=True)
        if delivery.status in {"sent", "simulated", "bounced", "complained", "delivered"}:
            return
...
def delete_application(org_id, app_id):
    with SessionLocal() as db:
        ...
        sessions = list(db.scalars(select(InterviewSession.id).where(InterviewSession.application_id == app.id, InterviewSession.org_id == org_id)))
        invites = list(db.scalars(select(Invitation.id).where(Invitation.application_id == app.id, Invitation.org_id == org_id)))
    ...
    with SessionLocal.begin() as db:
        ...
        for delivery in deliveries:
            if app_id in delivery.dedupe_key or any(i in delivery.dedupe_key for i in invites):
                db.delete(delivery)
        db.execute(delete(Usage).where(Usage.application_id == app_id, Usage.org_id == org_id))
        db.execute(delete(AsyncJob).where(AsyncJob.org_id == org_id, AsyncJob.target_id.in_([app_id]+docs+sessions), AsyncJob.kind != "delete"))
        db.execute(delete(Application).where(Application.id == app_id, Application.org_id == org_id))
        if not db.scalar(select(Application.id).where(Application.candidate_id == candidate_id, Application.org_id == org_id)):
            db.execute(delete(Candidate).where(Candidate.id == candidate_id, Candidate.org_id == org_id))
        audit(db, org_id, "worker", "application.deleted", app_id, {"object_versions_removed": True, "checkpoints_removed": True})
...
def process_job(job_id):
    with SessionLocal.begin() as db:
        ...
        if not job or job.status in {"done", "dead"} or aware(job.available_at) > now():
            return False
        acquired = db.execute(update(AsyncJob).where(AsyncJob.id == job_id,
            or_(AsyncJob.locked_until.is_(None), AsyncJob.locked_until < now()), AsyncJob.status.in_(["pending", "running"]))
    ...
    try:
        handlers = {"extract": extract_document, "prepare": prepare_application, "email": send_delivery,
                    "evaluate": evaluate_application, "verify_recording": verify_recording, "delete": delete_application}
        if kind == "frame":
...
def sweep():
    with SessionLocal.begin() as db:
        active = db.scalars(select(InterviewSession).where(InterviewSession.status == "active", InterviewSession.deadline_at < now()).with_for_update(skip_locked=True)).all()
        for session in active:
            finish_session(db, session, "server_deadline")
        expired = db.scalars(select(Application).where(Application.delete_after < now(), Application.status != "deleting")).all()
        for app in expired:
            app.revoked, app.status = True, "deleting"
            enqueue(db, app.org_id, "delete", app.id, "delete:"+app.id)
        invites = db.scalars(select(Invitation).where(Invitation.revoked.is_(False), Invitation.consumed_at.is_(None),
            Invitation.expires_at > now(), Invitation.expires_at < now()+timedelta(days=1), Invitation.last_reminder_at.is_(None))).all()
        for invite in invites:
            app = owned(db, Application, invite.application_id, invite.org_id)
            person = owned(db, Candidate, app.candidate_id, invite.org_id)
            queue_email(db, invite.org_id, person.email, "reminder", "reminder:"+invite.id,
                "Your interview invitation expires soon", "Your Talyn interview invitation expires within 24 hours. Use the secure link in your original invitation email or contact the hiring team for accommodations.")
...
def dispatch():
    with SessionLocal() as db:
        jobs = db.scalars(select(AsyncJob).where(AsyncJob.status.in_(["pending", "running"]),
            AsyncJob.available_at <= now(), or_(AsyncJob.locked_until.is_(None), AsyncJob.locked_until < now()))
            .order_by(AsyncJob.created_at).limit(20)).all()
    ...
    for identifier, org_id, published_at in items:
        if settings().mode == "demo":
            ...
            aws("sqs").send_message(QueueUrl=settings().queue_url, MessageBody=json.dumps({"job_id": identifier, "org_id": org_id}))
            with SessionLocal.begin() as db:
...
def receive_delivery_events():
    if not settings().event_queue_url:
        return
    sqs = aws("sqs")
    for message in sqs.receive_message(QueueUrl=settings().event_queue_url, MaxNumberOfMessages=10, WaitTimeSeconds=1).get("Messages", []):
        ...
        with SessionLocal.begin() as db:
            ...
            if delivery and not db.scalar(select(DeliveryEvent.id).where(DeliveryEvent.provider_event_id == message["MessageId"])):
                db.add(DeliveryEvent(org_id=delivery.org_id, delivery_id=delivery.id, provider_event_id=message["MessageId"], kind=kind))
                delivery.status = {"bounce": "bounced", "complaint": "complained", "delivery": "delivered"}.get(kind, delivery.status)
...
def run_once():
    ...
    if settings().mode == "aws":
        sqs = aws("sqs")
        messages = sqs.receive_message(QueueUrl=settings().queue_url, MaxNumberOfMessages=5,
            WaitTimeSeconds=5, VisibilityTimeout=300).get("Messages", [])
