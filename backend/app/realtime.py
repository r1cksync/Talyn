    ...
    try:
        with SessionLocal() as db:
            ...
            session = db.scalar(select(InterviewSession).where(InterviewSession.application_id == app.id,
                InterviewSession.org_id == auth.org_id))
        ...
        if hello.get("type") != "start" or hello.get("sample_rate") != 16000 or not secrets.compare_digest(hello.get("csrf", ""), csrf):
        ...
        stream = await client.start_stream_transcription(language_code="en-US", media_sample_rate_hz=16000,
            media_encoding="pcm", enable_partial_results_stabilization=True, partial_results_stability="medium")
        ...
        class Handler(TranscriptResultStreamHandler):
            async def handle_transcript_event(self, event):
                for result in event.transcript.results:
                    ...
                    confidence = [float(i.confidence) for i in alternative.items or [] if getattr(i, "confidence", None) is not None]
                    ...
                    def persist():
                        with SessionLocal.begin() as db:
                            ...
                            if not live or live.connection_id != connection_id or live.turn != turn or application.revoked:
                                return None
                            segment = reconcile_segment(db, live, result_id=f"{connection_id}:{result.result_id}",
                                text=alternative.transcript, start_ms=offset_ms+int(result.start_time*1000),
                                end_ms=offset_ms+int(result.end_time*1000), final=not result.is_partial, quality=quality)
                            return {"type": "transcript", "result_id": segment.result_id, "id": segment.id, "text": segment.text,
                                    "final": segment.final, "revision": segment.revision, "turn": segment.turn,
        ...
        while True:
            ...
            if (now()-last_check).total_seconds() >= 5:
                with SessionLocal.begin() as db:
                    ...
                    if app.revoked or auth_check.revoked or live.status != "active" or aware(live.deadline_at) <= now() or expires <= now():
                        break
        ...
        try:
            await ws.send_json({"type": "error", "message": "Audio connection ended. Reconnect to continue; saved captions are preserved."})
        except Exception:
            pass
    finally:
        if stream:
        ...
        if sid:
            with SessionLocal.begin() as db:
                db.execute(update(InterviewSession).where(InterviewSession.id == sid,
                    InterviewSession.connection_id == connection_id).values(connection_id=None, connection_until=None))
                meter(db, oid, aid, "transcription_seconds", (now()-started).total_seconds(), "transcribe:"+connection_id)
        try:
            await ws.close()
        except Exception:
            pass
