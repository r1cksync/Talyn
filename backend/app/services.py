...
from .models import (Answer, Application, Candidate, Consent, Criterion, Evaluation, Evidence, Extraction,
                     InterviewSession, Job, Membership, Organization, Plan, Question, Report,
                     RubricVersion, TranscriptSegment, Transition, now)
...
def questions_for(db, application_id, org_id):
    ...
    questions = db.scalars(select(Question).where(Question.plan_id == plan.id, Question.org_id == org_id).order_by(Question.position)).all()
    return plan, questions


def question_dict(q):
    return {"text": q.text, "competency": q.competency, "kind": q.kind, "expected_evidence": q.expected_evidence,
            "time_limit_seconds": q.time_limit_seconds, "max_followups": q.max_followups, "source_refs": q.source_refs}
...
def prepare_application(org_id, application_id):
    ...
    with SessionLocal() as db:
        ...
        extracts = db.scalars(select(Extraction).join(Document).where(Document.application_id == app.id, Extraction.org_id == org_id)).all()
        criteria = db.scalars(select(Criterion).where(Criterion.job_id == job.id, Criterion.org_id == org_id)).all()
        initial = {"org_id": org_id, "application_id": app.id,
                   "job": {"title": job.title, "description": job.description, "skills": job.skills,
                       "duration_minutes": job.duration_minutes, "criteria": [{"name": c.name, "description": c.description,
                           "weight": c.weight, "anchors": c.anchors} for c in criteria]},
                   "documents": [{"text": e.text, "sources": [{**s, "ref": f"{e.document_id}:{s['ref']}"}
                                  for s in e.sources]} for e in extracts]}
    ...
    with SessionLocal.begin() as db:
        ...
        if existing:
            return
        plan = Plan(org_id=org_id, application_id=app.id, model_id="synthetic-demo" if settings().mode == "demo" else settings().bedrock_model_id,
...
def start_session(db, auth):
    ...
    consent = db.scalar(select(Consent).where(Consent.application_id == app.id, Consent.org_id == app.org_id,
                                             Consent.policy_version == "2026-09-v1"))
    ...
    result = db.execute(update(Organization).where(Organization.id == app.org_id,
        ...
        .values(active_sessions=Organization.active_sessions+1, used_minutes=Organization.used_minutes+job.duration_minutes))
    if not result.rowcount:
        raise HTTPException(429, "Organization interview capacity or monthly minute limit reached")
    session = InterviewSession(org_id=app.org_id, application_id=app.id, status="active", started_at=now(),
                               deadline_at=now() + timedelta(minutes=job.duration_minutes))
    db.add(session)
    db.flush()
    db.add(Transition(org_id=app.org_id, session_id=session.id, from_state="ready", to_state="active", reason="candidate_start"))
...
def session_state(db, session):
    ...
    return {"id": session.id, "status": session.status, "question_index": session.question_index,
            "question_count": len(questions), "question": {"id": q.id, "text": text, "competency": q.competency} if q else None,
            "turn": session.turn, "deadline_at": aware(session.deadline_at).isoformat() if session.deadline_at else None,
            "started_at": aware(session.started_at).isoformat() if session.started_at else None,
            "server_time": now().isoformat(), "silence_seconds": settings().silence_seconds}
...
def reconcile_segment(db, session, *, result_id, text, start_ms, end_ms, final, quality="unknown"):
    ...
    segment = db.scalar(select(TranscriptSegment).where(TranscriptSegment.session_id == session.id,
        TranscriptSegment.org_id == session.org_id, TranscriptSegment.result_id == result_id))
    ...
    if not segment:
        segment = TranscriptSegment(org_id=session.org_id, session_id=session.id, question_id=questions[session.question_index].id,
                                    result_id=result_id, turn=session.turn, revision=0)
        db.add(segment)
...
def finish_session(db, session, reason="completed"):
    ...
    db.execute(update(Organization).where(Organization.id == app.org_id).values(
    ...
    db.add(Transition(org_id=app.org_id, session_id=session.id, from_state="active", to_state="completed", reason=reason))
...
def complete_turn(org_id, session_id, turn):
    ...
    with SessionLocal() as db:
        ...
        segments = db.scalars(select(TranscriptSegment).where(TranscriptSegment.session_id == session.id,
            TranscriptSegment.org_id == org_id, TranscriptSegment.turn == turn, TranscriptSegment.final.is_(True),
            TranscriptSegment.speaker == "candidate").order_by(TranscriptSegment.start_ms)).all()
        initial = {"org_id": org_id, "application_id": session.application_id, "session_id": session.id, "turn": turn,
                   "question": question_dict(q), "answer": " ".join(s.text for s in segments), "followups": session.followups,
                   "remaining_seconds": max(0, int((aware(session.deadline_at)-now()).total_seconds()))}
        segment_ids = [s.id for s in segments]
        quality = "answered" if segments and all(s.quality not in {"poor", "unknown"} for s in segments) else "technical_or_missing_audio"
    with checkpoint_store() as saver:
        state = invoke_recoverable(execution_graph(saver), initial, thread_id(org_id, initial["application_id"], f"turn-{turn}"))
    with SessionLocal.begin() as db:
        session = owned(db, InterviewSession, session_id, org_id, lock=True)
        if session.turn != turn or session.status != "active":
            return session_state(db, session)
        db.add(Answer(org_id=org_id, session_id=session.id, question_id=q.id, turn=turn, segment_ids=segment_ids, status=quality))
        session.turn += 1
...
def evaluate_application(org_id, application_id):
    with SessionLocal() as db:
        app = owned(db, Application, application_id, org_id)
        session = db.scalar(select(InterviewSession).where(InterviewSession.application_id == app.id, InterviewSession.org_id == org_id))
        ...
        rubric = db.scalar(select(RubricVersion).where(RubricVersion.plan_id == plan.id, RubricVersion.org_id == org_id,
                                                       RubricVersion.version == plan.version))
        by_id = {q.id: q for q in questions}
        segments = db.scalars(select(TranscriptSegment).where(TranscriptSegment.session_id == session.id,
            TranscriptSegment.org_id == org_id).order_by(TranscriptSegment.start_ms)).all()
        answers = db.scalars(select(Answer).where(Answer.session_id == session.id, Answer.org_id == org_id)).all()
        initial = {"org_id": org_id, "application_id": app.id, "session_id": session.id,
            "rubric": rubric.dimensions,
            "segments": [{"id": s.id, "text": s.text, "speaker": s.speaker, "final": s.final, "quality": s.quality,
                          "competency": by_id[s.question_id].competency} for s in segments],
            "answers": [{"label": by_id[a.question_id].competency, "status": a.status} for a in answers]
                + [{"label": q.competency, "status": "unanswered"} for q in questions if q.id not in {a.question_id for a in answers}]}
    with checkpoint_store() as saver:
    ...
    with SessionLocal.begin() as db:
        ...
        if db.scalar(select(Report.id).where(Report.session_id == initial["session_id"], Report.org_id == org_id)):
            return
        ev = Evaluation(org_id=org_id, session_id=initial["session_id"], dimensions=state["evaluation"]["dimensions"],
                        completeness="incomplete" if state["incomplete"] else "complete")
        db.add(ev)
        db.flush()
        for dimension in ev.dimensions:
            for ref in dimension["evidence"]:
                db.add(Evidence(org_id=org_id, evaluation_id=ev.id, dimension=dimension["name"],
                                segment_id=ref["segment_id"], quote=ref["quote"]))
        ...
        for member in managers:
            queue_email(db, org_id, member.email, "manager_report", f"manager-report:{app.id}:1:{member.id}",
                "Interview report ready for review", f"An interview report is ready. Sign in to review the evidence and make your own decision: {settings().public_url}/?report={app.id}")
        app.status = "reported"
