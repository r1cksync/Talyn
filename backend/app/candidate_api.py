...
from .models import (Application, Candidate, Consent, EmailChallenge, InterviewSession, Invitation, Job,
                     RecordingManifest, RecordingObject, Report, TranscriptSegment, VideoObservation, now)
from .outbox import audit, enqueue, meter, queue_email
from .schemas import (AccommodationInput, ChallengeInput, ClipInput, ConsentInput, DemoAnswer, ExplanationInput,
                      InviteInput, ManifestInput, StrictModel, TurnInput)
...
def issue_challenge(db, invite):
    ...
    challenge = EmailChallenge(org_id=invite.org_id, invitation_id=invite.id,
        code_hash=secret_digest(code), expires_at=now()+timedelta(minutes=10))
    db.add(challenge)
    db.flush()
    app = owned(db, Application, invite.application_id, invite.org_id)
    person = owned(db, Candidate, app.candidate_id, invite.org_id)
    queue_email(db, invite.org_id, person.email, "verification", "verify:" + challenge.id,
                "Your Talyn verification code", f"Your code is {code}. It expires in 10 minutes. Do not share this code.")
...
def request_resume(data: ResumeInput, request: Request, db: Session = Depends(get_db)):
    ...
    if app and not app.revoked and (not app.delete_after or aware(app.delete_after) > now()):
        ...
        invite = db.scalar(select(Invitation).where(Invitation.application_id == app.id, Invitation.org_id == app.org_id,
                                                    Invitation.revoked.is_(False), Invitation.consumed_at.is_not(None)))
...
def verify_access(data: ChallengeInput, request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    result = db.execute(update(EmailChallenge).where(EmailChallenge.id == challenge.id,
        EmailChallenge.consumed.is_(False)).values(consumed=True))
    if not result.rowcount:
        raise HTTPException(401, "Verification code already used")
    if not invite.consumed_at:
        invite.consumed_at = now()
    person = owned(db, Candidate, app.candidate_id, invite.org_id)
    session, token = create_session(db, kind="candidate", subject=person.id, email=person.email,
        org_id=invite.org_id, application_id=invite.application_id)
...
def candidate_info(auth=Depends(candidate), db: Session = Depends(get_db)):
    ...
    session = db.scalar(select(InterviewSession).where(InterviewSession.application_id == app.id, InterviewSession.org_id == app.org_id))
    return {"csrf": auth.csrf, "application_id": app.id, "name": person.name, "job_title": job.title,
            "duration_minutes": job.duration_minutes, "recording_required": app.recording_required,
            "accommodation": app.accommodation, "mode": settings().mode,
            "policy_version": "2026-09-v1", "frame_interval_seconds": settings().frame_interval_seconds,
...
def current_session(db, auth, active=False):
    session = db.scalar(select(InterviewSession).where(InterviewSession.application_id == auth.application_id,
        InterviewSession.org_id == auth.org_id).with_for_update())
...
def get_transcript(auth=Depends(candidate), db: Session = Depends(get_db)):
    ...
    segments = db.scalars(select(TranscriptSegment).where(TranscriptSegment.session_id == session.id,
        TranscriptSegment.org_id == auth.org_id).order_by(TranscriptSegment.start_ms)).all()
    return [{"id": s.id, "result_id": s.result_id, "text": s.text, "final": s.final, "revision": s.revision,
             "start_ms": s.start_ms, "end_ms": s.end_ms, "turn": s.turn} for s in segments]
...
def demo_answer(data: DemoAnswer, auth=Depends(candidate), db: Session = Depends(get_db)):
    ...
    segment = reconcile_segment(db, session, result_id=data.result_id, text=data.text, start_ms=max(0, offset-2000),
                                end_ms=offset, final=data.final, quality="synthetic")
...
def recording_upload(data: ClipInput, auth=Depends(candidate), db: Session = Depends(get_db)):
    session = current_session(db, auth)
    consent = db.scalar(select(Consent).where(Consent.application_id == auth.application_id, Consent.org_id == auth.org_id))
    if not consent or not consent.recording:
        raise HTTPException(403, "Recording consent required")
    if session.status not in {"active", "completed"} or data.end_ms <= data.start_ms or data.end_ms-data.start_ms > 35000:
    ...
    existing = db.scalar(select(RecordingObject).where(RecordingObject.session_id == session.id,
        RecordingObject.org_id == auth.org_id, RecordingObject.sequence == data.sequence))
    if existing:
        ...
        db.add(obj)
        db.flush()
    return {"id": obj.id, "verified": obj.verified, "upload": storage.upload_url(obj.object_key, obj.content_type, obj.size, obj.sha256)}
...
def finalize_manifest(data: ManifestInput, auth=Depends(candidate), db: Session = Depends(get_db)):
    session = current_session(db, auth)
    rows = db.scalars(select(RecordingObject).where(RecordingObject.session_id == session.id,
        RecordingObject.org_id == auth.org_id).order_by(RecordingObject.sequence)).all()
...
def recordings_status(auth=Depends(candidate), db: Session = Depends(get_db)):
    session = current_session(db, auth)
    rows = db.scalars(select(RecordingObject).where(RecordingObject.session_id == session.id, RecordingObject.org_id == auth.org_id)).all()
    return {"next_sequence": max([r.sequence for r in rows], default=-1)+1,
...
def observations(auth=Depends(candidate), db: Session = Depends(get_db)):
    session = current_session(db, auth)
    rows = db.scalars(select(VideoObservation).where(VideoObservation.org_id == auth.org_id,
        VideoObservation.session_id == session.id, VideoObservation.visible.is_(True))).all()
    return [{"id": o.id, "kind": o.kind, "start_ms": o.start_ms, "end_ms": o.end_ms, "explanation": o.explanation} for o in rows]


@router.patch("/observations/{observation_id}/explanation")
def explain_observation(observation_id: str, data: ExplanationInput, auth=Depends(candidate), db: Session = Depends(get_db)):
    session = current_session(db, auth)
...
def candidate_report(auth=Depends(candidate), db: Session = Depends(get_db)):
    ...
    report = db.scalar(select(Report).where(Report.session_id == session.id, Report.org_id == auth.org_id,
        Report.audience == "candidate").order_by(Report.version.desc()))
    if not report:
