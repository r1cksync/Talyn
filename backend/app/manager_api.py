...
from .models import (Application, AsyncJob, AuthSession, Campaign, Candidate, Criterion, Document, EmailDelivery,
    Extraction, InterviewSession, Invitation, Job, Membership, Organization, Plan, Question, RecordingManifest,
    RecordingObject, Report, RubricVersion, TranscriptSegment, Usage, VideoObservation, now)
from .outbox import audit, enqueue, queue_email
from .schemas import (CampaignInput, CandidateInput, DocumentInput, ExplanationInput, JobInput, MemberInput, NoteInput,
                      OrgInput, OrgSettingsInput, PlanEdit)
...
def create_org(data: OrgInput, auth=Depends(manager), db: Session = Depends(get_db)):
    org = Organization(name=data.name, retention_days=settings().retention_days, max_minutes=settings().max_monthly_minutes,
...
def get_org(auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    usage = db.execute(select(Usage.meter, func.sum(Usage.quantity)).where(Usage.org_id == org.id).group_by(Usage.meter)).all()
    return {**as_dict(org, "id name retention_days max_minutes max_concurrent used_minutes active_sessions"), "role": member.role,
            "members": [as_dict(m, "id email role") for m in members], "usage": {key: value for key, value in usage}}
...
def add_member(data: MemberInput, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    if settings().mode == "aws":
        users = aws("cognito-idp").list_users(UserPoolId=settings().cognito_pool_id, Filter=f'email = "{str(data.email)}"', Limit=2)["Users"]
        if len(users) != 1:
        ...
        session = db.scalar(select(AuthSession).where(AuthSession.email == str(data.email), AuthSession.kind == "manager"))
        if not session:
...
def job_detail(job_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    return {**as_dict(job, "id title description seniority skills duration_minutes status"),
...
def add_candidate(db, org_id, job_id, data):
    ...
    if not person:
        ...
        db.add(person)
        db.flush()
    app = db.scalar(select(Application).where(Application.org_id == org_id, Application.job_id == job_id, Application.candidate_id == person.id))
    if not app:
        app = Application(org_id=org_id, job_id=job_id, candidate_id=person.id)
        db.add(app)
        db.flush()
    return app
...
def applications(job_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    membership(db, auth)
    owned(db, Job, job_id, auth.org_id)
    apps = db.scalars(select(Application).where(Application.job_id == job_id, Application.org_id == auth.org_id).order_by(Application.created_at)).all()
    result = []
    for app in apps:
        ...
        result.append({**as_dict(app, "id status accommodation recording_required revoked"), "name": candidate.name, "email": candidate.email,
            "documents": [as_dict(d, "id filename status error purpose") for d in docs], "plan_approved": bool(plan and plan.approved),
...
def upload_document(application_id: str, data: DocumentInput, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    count = db.scalar(select(func.count(Document.id)).where(Document.application_id == app.id, Document.org_id == auth.org_id))
    if count >= 4:
...
def extraction(document_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    return {"status": doc.status, "error": doc.error, "sources": result.sources if result else [], "warnings": result.warnings if result else []}
...
def plan_detail(application_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    rubric = db.scalar(select(RubricVersion).where(RubricVersion.plan_id == plan.id, RubricVersion.org_id == auth.org_id,
                                                  RubricVersion.version == plan.version))
    return {**as_dict(plan, "id version approved warnings model_id"), "questions": [question_dict(q) for q in questions], "dimensions": rubric.dimensions}
...
def edit_plan(application_id: str, data: PlanEdit, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    job_names = set(db.scalars(select(Criterion.name).where(Criterion.job_id == app.job_id, Criterion.org_id == auth.org_id)))
    if names != job_names or len(names) != len(data.dimensions) or any(q.competency not in names for q in data.questions):
    ...
    db.add(RubricVersion(org_id=auth.org_id, plan_id=plan.id, version=plan.version,
        dimensions=[d.model_dump() for d in data.dimensions], approved_by=auth.subject if data.approve else None))
...
def launch(job_id: str, data: CampaignInput, auth=Depends(manager), db: Session = Depends(get_db)):
    membership(db, auth, write=True)
    owned(db, Job, job_id, auth.org_id)
    existing = db.scalar(select(Campaign).where(Campaign.org_id == auth.org_id, Campaign.idempotency_key == data.idempotency_key))
    if existing:
    ...
    for app in apps:
        ...
        invitation = Invitation(org_id=auth.org_id, campaign_id=campaign.id, application_id=app.id,
                                token_hash=digest(token), expires_at=now()+timedelta(days=7))
        db.add(invitation)
        db.flush()
        queue_email(db, auth.org_id, person.email, "invitation", "invite:"+invitation.id, "Your Talyn interview invitation",
...
def revoke(application_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    app = owned(db, Application, application_id, auth.org_id, lock=True)
    app.revoked = True
    for invitation in db.scalars(select(Invitation).where(Invitation.application_id == app.id, Invitation.org_id == auth.org_id)):
        invitation.revoked = True
    for session in db.scalars(select(AuthSession).where(AuthSession.application_id == app.id, AuthSession.org_id == auth.org_id)):
        session.revoked = True
    active = db.scalar(select(InterviewSession).where(InterviewSession.application_id == app.id, InterviewSession.org_id == auth.org_id))
...
def report_detail(application_id: str, auth=Depends(manager), db: Session = Depends(get_db)):
    membership(db, auth)
    app = owned(db, Application, application_id, auth.org_id)
    session = db.scalar(select(InterviewSession).where(InterviewSession.application_id == app.id, InterviewSession.org_id == auth.org_id))
    if not session:
        raise HTTPException(404, "Interview has not started")
    report = db.scalar(select(Report).where(Report.session_id == session.id, Report.org_id == auth.org_id,
                                          Report.audience == "manager").order_by(Report.version.desc()))
    if not report:
        raise HTTPException(404, "Report is not ready")
    segments = db.scalars(select(TranscriptSegment).where(TranscriptSegment.session_id == session.id,
        TranscriptSegment.org_id == auth.org_id, TranscriptSegment.final.is_(True)).order_by(TranscriptSegment.start_ms)).all()
    observations = db.scalars(select(VideoObservation).where(VideoObservation.session_id == session.id,
        VideoObservation.org_id == auth.org_id, VideoObservation.visible.is_(True)).order_by(VideoObservation.start_ms)).all()
    clips = db.scalars(select(RecordingObject).where(RecordingObject.session_id == session.id,
        RecordingObject.org_id == auth.org_id, RecordingObject.verified.is_(True)).order_by(RecordingObject.sequence)).all()
    manifest = db.scalar(select(RecordingManifest).where(RecordingManifest.session_id == session.id, RecordingManifest.org_id == auth.org_id))
    audit(db, auth.org_id, auth.subject, "report.viewed", report.id)
    return {**report.content, **as_dict(report, "id version manager_notes decision"), "application_id": app.id,
        "transcript": [as_dict(s, "id text speaker start_ms end_ms quality question_id") for s in segments],
        "observations": [as_dict(o, "id kind start_ms end_ms confidence sample_count dismissed explanation") for o in observations],
        "recordings": [{**as_dict(c, "sequence start_ms end_ms content_type"), "url": storage.download_url(c.object_key, c.content_type)} for c in clips],
...
def annotate_report(report_id: str, data: NoteInput, auth=Depends(manager), db: Session = Depends(get_db)):
    ...
    latest = db.scalar(select(func.max(Report.version)).where(Report.session_id == report.session_id,
        Report.org_id == auth.org_id, Report.audience == "manager"))
    if report.version != latest:
        raise HTTPException(409, "Reload the latest report")
    new = Report(org_id=auth.org_id, session_id=report.session_id, audience="manager", version=latest+1,
                 content=report.content, manager_notes=data.notes, decision=data.decision)
    db.add(new)
    db.flush()
...
def dismiss_observation(observation_id: str, data: ExplanationInput, auth=Depends(manager), db: Session = Depends(get_db)):
    membership(db, auth, write=True)
...
def deliveries(auth=Depends(manager), db: Session = Depends(get_db)):
    membership(db, auth)
    rows = db.scalars(select(EmailDelivery).where(EmailDelivery.org_id == auth.org_id).order_by(EmailDelivery.created_at.desc()).limit(200)).all()
    return [{**as_dict(d, "id recipient kind status attempts error created_at"),
             **({"preview": json.loads(decrypt(d.payload_ciphertext))} if settings().mode == "demo" else {})} for d in rows]
