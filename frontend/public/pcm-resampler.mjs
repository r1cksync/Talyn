  ...
  push(input) {
    ...
    while (this.position + 1 < this.samples.length) {
      ...
      const value = Math.max(
        -1,
        Math.min(1, this.samples[i] * (1 - t) + this.samples[i + 1] * t),
      );
      this.packet[this.used++] = Math.round(
        value < 0 ? value * 32768 : value * 32767,
      );
      if (this.used === this.packet.length) {
        const buffer = new ArrayBuffer(this.packet.length * 2),
          view = new DataView(buffer);
        for (let n = 0; n < this.packet.length; n++)
          view.setInt16(n * 2, this.packet[n], true);
        packets.push(buffer);
        this.used = 0;
      }
